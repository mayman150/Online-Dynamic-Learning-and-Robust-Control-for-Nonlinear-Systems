...
%% ---------------- ==> Simulation time == tfinal == 10
%% ---------------- ==> Simulation with x-y and alpha-Beta exponential paths with kinematic uncertainty and disturbance for 8-cables
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all;
clear;
clc;
...
%% Neural networks intialization
%% Weights of RBF Network of notation a
Wt = zeros(6*hiddenlayer_a + 6*hiddenlayer_c,1);
...
d_xd = [ -1.5*exp(-r*T)+1.8*(1/1.2)*exp(-(r*T)/1.2);
         -1.5*exp(-r*T)+1.8*(1/1.2)*exp(-(r*T)/1.2);
         0;
         -exp(-r*T)+exp(-(r*T)/1.2);
         -exp(-r*T)+exp(-(r*T)/1.2);
         0];

dd_xd = [1.5*exp(-r*T)-1.8*(1/1.2)*(1/1.2)*exp(-(r*T)/1.2); 
...
tau = us*tanh(((u)-[up,up,up,up,up,up,up,up]')/us)+up;
tau_d = tau;
Tau(:,r) = tau;%#ok
