...
%% ---------------- ==> Simulation with sinusoidal path without kinematic uncertainty for 8-cables
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all;
clear;
clc;
...
%hiddenlayer_a = 256;      %% L_a : number of hiddenlayers of a
...
xd = [0.2*sin(1*r*T);
      ...
      0];
Xd(:,r) = xd;
d_xd = [ 0.2*1*cos(1*r*T);
         -0.2*1*sin(1*r*T);
         0.2*3*cos(3*r*T);
         0;
         0;
         0];
dd_xd = [ -0.2*1*1*sin(1*r*T);
          -0.2*1*1*cos(1*r*T);
          -0.2*3*3*sin(3*r*T);
          0;
          0;
...
Z_a = [x; d_x; xd; d_xd; dd_xd; tau_d];%[e_1;e_2;dd_x_d;dd_xd; tau]; %[x; d_x; xd; d_xd; dd_xd]; %
eta_a_p2 = eta_a.^2;
